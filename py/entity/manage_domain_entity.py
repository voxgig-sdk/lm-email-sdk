...
class ManageDomainEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return ManageDomainEntity(self._client, opts)
    ...
    def data_get(self) -> ManageDomain:
    ...
    def match_get(self) -> ManageDomain:
    ...
    def remove(self, reqmatch=None, ctrl=None) -> ManageDomain:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key removes with no
        # match. Treat None as an empty match so client.ManageDomain().remove()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
