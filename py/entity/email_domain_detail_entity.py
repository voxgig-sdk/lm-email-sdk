...
class EmailDomainDetailEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return EmailDomainDetailEntity(self._client, opts)
    ...
    def data_get(self) -> EmailDomainDetail:
    ...
    def match_get(self) -> EmailDomainDetail:
    ...
    def load(self, reqmatch=None, ctrl=None) -> EmailDomainDetail:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.EmailDomainDetail().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
