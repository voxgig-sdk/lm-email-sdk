...
from lmemail_types import (
    EmailDomainVerify,
    EmailDomainVerifyLoadMatch,
)
...
class EmailDomainVerifyEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return EmailDomainVerifyEntity(self._client, opts)
    ...
    def data_get(self) -> EmailDomainVerify:
    ...
    def match_get(self) -> EmailDomainVerify:
    ...
    def load(self, reqmatch=None, ctrl=None) -> EmailDomainVerify:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.EmailDomainVerify().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
