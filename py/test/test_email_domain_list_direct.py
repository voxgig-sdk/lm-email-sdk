...
class TestEmailDomainListDirect:

    def test_should_direct_list_email_domain_list(self):
        setup = _email_domain_list_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-email_domain_list", "live" if setup["live"] else "unit")
        if _skip:
...
def _email_domain_list_direct_setup(mockres):
