// Command LmEmail-mcp is an MCP server exposing the LmEmail
// SDK as MCP tools.
//
// Two tools are registered:
//
//	LmEmail_list  — list records of an entity
//	LmEmail_load  — load a single record of an entity
//
...
func main() {
	...
	// Configure from the environment: LM_EMAIL_APIKEY carries the API key and
	// LM_EMAIL_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("LM_EMAIL_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("LM_EMAIL_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewLmEmailSDK(opts)
