...
[MCP](https://modelcontextprotocol.io) server exposing the LmEmail SDK as
two agent tools — `lm-email_list` and `lm-email_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/lm-email-mcp)
make build
...
claude mcp add --scope user lm-email \
  -- /absolute/path/to/lm-email-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./lm-email-mcp -transport http -addr :8080
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/lm-email-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user lm-email \
     -- "$PWD"/dist/*/lm-email-mcp -transport stdio
   ```
...
| Tool | Args | Returns |
|------|------|---------|
| `lm-email_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `lm-email_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `LM_EMAIL_APIKEY` | API key sent with every request. |
| `LM_EMAIL_BASE` | Optional override of the API base URL. |
...
```sh
./lm-email-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"lm-email_load","arguments":{"entity":"email_domain_detail","query":{"id":1}}}}'
```
