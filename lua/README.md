...
```lua
local emaildomainverify, err = client:EmailDomainVerify():load({ domain_id = 1, type = "example_type" })
if err then error(err) end
print(emaildomainverify)
```
...
Create an instance: `local email_domain_verify = client:EmailDomainVerify(nil)`
...
```lua
local email_domain_verify, err = client:EmailDomainVerify():load({ domain_id = 1, type = "type" })
```


### ManageDomain

Create an instance: `local manage_domain = client:ManageDomain(nil)`
...
```
lua/
├── lm-email_sdk.lua    -- Main SDK module
...
The main module (`lm-email_sdk`) exports the SDK constructor
