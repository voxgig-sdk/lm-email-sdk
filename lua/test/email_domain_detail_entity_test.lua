...
describe("EmailDomainDetailEntity", function()
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set LMEMAIL_TEST_EMAIL_DOMAIN_DETAIL_ENTID JSON to run live")
      return
    end
    ...
    assert.is_nil(err)
    local email_domain_detail_ref01_data_dt0_load_result = helpers.to_map(email_domain_detail_ref01_data_dt0_loaded)
...
function email_domain_detail_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("LMEMAIL_TEST_EMAIL_DOMAIN_DETAIL_ENTID")
