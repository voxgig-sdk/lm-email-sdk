...
function LmEmailSDK:_op_denied(op)
  local allow = vs.getpath(self.options, "allow.op")
  if type(allow) ~= "string" then allow = "" end
  return {
    ok = false,
    err = "LmEmailSDK: " .. op .. ": operation not allowed by" ..
...
-- by passing it.
function LmEmailSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function LmEmailSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "LmEmailSDK: graphql: " .. msg
    res.graphql = errors
  end
