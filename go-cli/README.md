...
AQL-driven command-line client **and** interactive REPL for the LmEmail
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/lm-email-cli)
make build
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/lm-email-cli
   ```
...
Run with no arguments to open a REPL (prompt `lm-email>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an AQL atom (e.g. `email_create_domain`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/lm-email-cli`. |
...
(`./lm-email-cli <expr>`) and interactively (the REPL), and expressions compose the
