...
class EmailDomainVerifyEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set LMEMAIL_TEST_EMAIL_DOMAIN_VERIFY_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def email_domain_verify_basic_setup(extra)
  ...
  entid_env_raw = ENV["LMEMAIL_TEST_EMAIL_DOMAIN_VERIFY_ENTID"]
