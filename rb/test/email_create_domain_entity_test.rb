...
class EmailCreateDomainEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set LM_EMAIL_TEST_EMAIL_CREATE_DOMAIN_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    email_create_domain_ref01_data = Helpers.to_map(email_create_domain_ref01_data_result.respond_to?(:data_get) ? email_create_domain_ref01_data_result.data_get : email_create_domain_ref01_data_result)
...
def email_create_domain_basic_setup(extra)
  ...
  entid_env_raw = ENV["LM_EMAIL_TEST_EMAIL_CREATE_DOMAIN_ENTID"]
