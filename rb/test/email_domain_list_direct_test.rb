...
class EmailDomainListDirectTest < Minitest::Test
  def test_direct_list_email_domain_list
    setup = email_domain_list_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-email_domain_list", setup[:live] ? "live" : "unit")
    if _should_skip
...
def email_domain_list_direct_setup(mockres)
