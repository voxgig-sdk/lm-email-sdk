...
require_relative 'LmEmail_types'
...
class LmEmailSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = LmEmailHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = LmEmailHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, LmEmailFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = LmEmailHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = LmEmailHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = LmEmailHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => LmEmailError.new(
        "#{op}_allow",
        "LmEmailSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = LmEmailHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = LmEmailError.new(
        "graphql_error", "LmEmailSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def EmailDomainVerify(data = nil)
    ...
    EmailDomainVerifyEntity.new(self, data)
  end


  # Canonical facade: client.ManageDomain.list / client.ManageDomain.load({ "id" => ... })
  def ManageDomain(data = nil)
    require_relative 'entity/manage_domain_entity'
    ManageDomainEntity.new(self, data)
  end
