...
class EmailDomainVerifyEntity
  ...
  def make
    opts = @_entopts.dup
    EmailDomainVerifyEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = LmEmailHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [EmailDomainVerify, Hash] the current EmailDomainVerify data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = LmEmailHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of EmailDomainVerify fields)
  def match_get
  ...
  #
  # @param reqmatch [EmailDomainVerifyLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.EmailDomainVerify.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [EmailDomainVerify, Hash] the loaded EmailDomainVerify; raises LmEmailError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = LmEmailHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
