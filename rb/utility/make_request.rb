...
module LmEmailUtilities
  MakeRequest = ->(ctx) {
    ...
    if fetch_err
      ...
      response = LmEmailResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = LmEmailResponse.new(fetched)
    else
