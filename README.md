...
Learn more about MyLINK EMAIL API at [linkmobility.com](https://linkmobility.com).

This is an unofficial SDK for the MyLINK EMAIL public API, generated by Voxgig with [`@voxgig/sdkgen`](https://github.com/voxgig/sdkgen). It is not affiliated with, endorsed by, or sponsored by the upstream API provider.
...
```json
{
  "mcpServers": {
    "lm-email": {
      "command": "/abs/path/to/lm-email-mcp"
    }
  }
}
```
...
```go
import sdk "github.com/voxgig-sdk/lm-email-sdk/go"
...
emailDomainVerify, err := client.EmailDomainVerify(nil).Load(
    map[string]any{"domain_id": 1}, nil,
)
if err != nil {
    panic(err)
}
...
Generated from the MyLINK EMAIL API OpenAPI spec by [@voxgig/sdkgen](https://github.com/voxgig/sdkgen).
