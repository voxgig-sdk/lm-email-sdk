...
class LmEmailSDK {
    ...
    async graphql(query, variables, ctrl) {
        ...
        if (null != errors && Array.isArray(errors) && 0 < errors.length) {
            ...
            const err = new Error('LmEmailSDK: graphql: ' +
    ...
    // The argument is the entity OPTIONS object (passed to the entity
    // constructor as entopts), not initial entity data.
    EmailDomainVerify(entopts) {
        const self = this;
        return new EmailDomainVerifyEntity_1.EmailDomainVerifyEntity(self, entopts);
    }
    // Entity access: `client.ManageDomain().list()` / `client.ManageDomain().load({ id })`.
    // The argument is the entity OPTIONS object (passed to the entity
    // constructor as entopts), not initial entity data.
    ManageDomain(entopts) {
        const self = this;
        return new ManageDomainEntity_1.ManageDomainEntity(self, entopts);
    }
