...
const TEST_SEED = { "entity": { "email_create_domain": { "test01": { "id": "test01" } }, "email_domain_detail": { "test01": { "id": "test01" } }, "email_domain_list": { "test01": { "id": "test01" } }, "email_domain_verify": { "test01": { "id": "test01" } }, "manage_domain": { "test01": { "id": "test01" } }, "send_message": { "test01": { "id": "test01" } } } };
...
function isRunnable(code) {
    return (/new\s+LmEmailSDK\b/.test(code) ||
        /\bLmEmailSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+LmEmailSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/LmEmailSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
