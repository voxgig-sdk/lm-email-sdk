...
describe('EmailDomainVerifyDirect', async () => {
  ...
  test('direct-load-email_domain_verify', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-email_domain_verify', setup.live)) return
    if (skipIfMissingIds(t, setup, ["domain_id01"])) return
