...
// LmEmail SDK EmailDomainDetail entity
...
class EmailDomainDetailEntity
{
    ...
    public function __construct($client, ?array $entopts = null)
    {
        ...
        $this->_name = "email_domain_detail";
        $this->_client = $client;
    ...
    {
        $opts = $this->_entopts;
        return new EmailDomainDetailEntity($this->_client, $opts);
    }

    /**
     * @param EmailDomainDetail|array $args EmailDomainDetail data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = LmEmailHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return EmailDomainDetail|array The current EmailDomainDetail data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of EmailDomainDetail fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = LmEmailHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of EmailDomainDetail fields).
     */
    ...
    /**
     * Load a single EmailDomainDetail.
     *
     * @param EmailDomainDetailLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed EmailDomainDetailLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return EmailDomainDetail|array The loaded EmailDomainDetail as an assoc-array at the
     *   SDK boundary; throws LmEmailError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = LmEmailHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
