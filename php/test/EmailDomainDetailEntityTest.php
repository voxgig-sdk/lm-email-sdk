...
class EmailDomainDetailEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set LM_EMAIL_TEST_EMAIL_DOMAIN_DETAIL_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $email_domain_detail_ref01_data_dt0_load_result = Helpers::to_map(is_object($email_domain_detail_ref01_data_dt0_loaded) && method_exists($email_domain_detail_ref01_data_dt0_loaded, 'data_get') ? $email_domain_detail_ref01_data_dt0_loaded->data_get() : $email_domain_detail_ref01_data_dt0_loaded);
...
function email_domain_detail_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("LM_EMAIL_TEST_EMAIL_DOMAIN_DETAIL_ENTID");
