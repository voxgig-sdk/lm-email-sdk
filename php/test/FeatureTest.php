...
class FtCtrl extends LmEmailControl
{
...
class FtClient
{
    ...
    private LmEmailUtility $utility;

    public function __construct(LmEmailUtility $utility, array $options)
    {
    ...
    public function get_utility(): LmEmailUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (LmEmailContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (LmEmailContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = LmEmailConfig::make_feature($name);
    ...
    private static function buildUrl(LmEmailSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof LmEmailError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new LmEmailResponse($fetched);
        }
    ...
    private function fail(LmEmailContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (LmEmailContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
