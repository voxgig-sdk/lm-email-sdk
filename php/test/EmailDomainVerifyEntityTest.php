...
class EmailDomainVerifyEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = LmEmailSDK::test(null, null);
        $ent = $testsdk->EmailDomainVerify(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = email_domain_verify_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "email_domain_verify." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        $email_domain_verify_ref01_data = null;
        if (count($email_domain_verify_ref01_data_raw) > 0) {
            $email_domain_verify_ref01_data = Helpers::to_map($email_domain_verify_ref01_data_raw[0][1]);
        }

        // LOAD
        $email_domain_verify_ref01_ent = $client->EmailDomainVerify(null);
        $email_domain_verify_ref01_match_dt0 = [];
        $email_domain_verify_ref01_data_dt0_loaded = $email_domain_verify_ref01_ent->load($email_domain_verify_ref01_match_dt0, null);
        $this->assertNotNull($email_domain_verify_ref01_data_dt0_loaded);
...
function email_domain_verify_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/email_domain_verify/EmailDomainVerifyTestData.json';
