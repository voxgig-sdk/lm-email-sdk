...
class LmEmailMakeError
{
    public static function call(?LmEmailContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new LmEmailContext([], null);
        }
        $op = $ctx->op ?? new LmEmailOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new LmEmailResult([]);
        $result->ok = false;
