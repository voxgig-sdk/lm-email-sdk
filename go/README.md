...
## Install
```bash
go get github.com/voxgig-sdk/lm-email-sdk/go@latest
```
...
```bash
go mod edit -replace github.com/voxgig-sdk/lm-email-sdk/go=../lm-email-sdk/go
```
...
### Entity interface (LmEmailEntity)

All entities implement the `LmEmailEntity` interface.
...
Create an instance: `emailDomainVerify := client.EmailDomainVerify(nil)`
...
```go
emailDomainVerify, err := client.EmailDomainVerify(nil).Load(map[string]any{"domain_id": 1, "type": "type"}, nil)
if err != nil {
    panic(err)
}
...
Create an instance: `manageDomain := client.ManageDomain(nil)`
...
```
github.com/voxgig-sdk/lm-email-sdk/go/
├── lm-email.go        # Root package — type aliases and constructors
...
The root package (`github.com/voxgig-sdk/lm-email-sdk/go`) re-exports everything needed
