...
func TestEmailDomainDetailEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set LMEMAIL_TEST_EMAIL_DOMAIN_DETAIL_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		emailDomainDetailRef01DataDt0LoadResult := core.ToMapAny(emailDomainDetailRef01DataDt0Loaded)
...
func email_domain_detailBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("LMEMAIL_TEST_EMAIL_DOMAIN_DETAIL_ENTID")
	...
	idmapResolved := core.ToMapAny(env["LMEMAIL_TEST_EMAIL_DOMAIN_DETAIL_ENTID"])
