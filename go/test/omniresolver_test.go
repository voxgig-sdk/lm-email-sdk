...
import (
	...
	omni "github.com/voxgig-sdk/lm-email-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.LmEmailError); is {
				out := map[string]any{
					"name":    "LmEmailError",
...
func omniCtx(arg any, client *sdk.LmEmailSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.LmEmailSDK); is {
			client = live
